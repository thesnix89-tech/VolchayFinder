...
static void ensureHwndResyncHook(QWindow* window, DockClickThroughState* state)
{
    ...
    // Qt may create the render child HWND shortly after first show — one resync is enough.
    QTimer::singleShot(400, window, [state, window]() {
                                         ...
                                         const QVariantList& hitRegions)
{
    ...
    const QList<QRect> nextClip = localRectsToScreen(window, clipLocal);
    const QList<QRect> nextHit = localRectsToScreen(window, hitLocal);
    if (data->clipRegions == nextClip && data->hitRegions == nextHit) {
        return;
    }
    data->clipRegions = nextClip;
    data->hitRegions = nextHit;
    applyRegionsToWindowTree(data);
}

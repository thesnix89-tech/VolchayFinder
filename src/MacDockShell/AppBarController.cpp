...
bool AppBarController::updateTopBarRect(void* hwnd, int height)
{
    ...
    const QRect proposed(abd.rc.left, abd.rc.top,
                         abd.rc.right - abd.rc.left,
                         abd.rc.bottom - abd.rc.top);

    // ABM_SETPOS triggers ABN_POSCHANGED, which re-enters this method. Bail out
    // when nothing moved so the notification cascade settles after one pass
    // instead of looping forever and freezing the GUI thread.
    if (m_hasAppliedRect && m_lastAppliedRect == proposed) {
        return false;
    }
...
void AppBarController::handleShellLayoutChange()
{
    ...
    // Only react to genuine layout changes. When the rect is unchanged this is a
    // self-induced ABN_POSCHANGED echo; swallowing it stops the feedback loop with
    // enforceTaskbarHidden() (which would otherwise EnumWindows on every echo).
    if (updateTopBarRect(m_hwnd, m_topBarHeight)) {
        emit layoutChanged();
    }
}

...
bool isAsciiUtf16LeChar(ushort code)
{
    return code >= 0x20 && code <= 0x7E;
}

void appendRegistryAsciiUtf16String(TaskbarFavoriteIndex* index, const QString& value)
{
    if (!index) {
        return;
    }
...
void appendStringsFromUtf16LeAsciiScan(const QByteArray& favorites, TaskbarFavoriteIndex* index)
{
    if (!index || favorites.size() < 4) {
        return;
    }

    const auto readCode = [&favorites](int offset) -> ushort {
        return static_cast<ushort>(
            static_cast<uchar>(favorites.at(offset))
            | (static_cast<ushort>(static_cast<uchar>(favorites.at(offset + 1))) << 8));
    };

    for (int align = 0; align <= 1; ++align) {
        int i = align;
        while (i + 1 < favorites.size()) {
            const ushort lead = readCode(i);
            if (!isAsciiUtf16LeChar(lead)) {
                i += 2;
                continue;
            }

            QString current;
            current.append(QChar(lead));
            i += 2;
            bool terminated = false;
            while (i + 1 < favorites.size()) {
                const ushort code = readCode(i);
                if (code == 0) {
                    terminated = true;
                    i += 2;
                    break;
                }
                if (!isAsciiUtf16LeChar(code)) {
                    current.clear();
                    break;
                }
                current.append(QChar(code));
            ...
            if (terminated && !current.isEmpty()) {
                appendRegistryAsciiUtf16String(index, current);
            }
        }
    }
}
...
TaskbarFavoriteIndex buildTaskbarFavoriteIndex(const QByteArray& favorites)
{
    ...
    appendStringsFromUtf16LeAsciiScan(favorites, &index);
...
PinnedShortcutEntry PinnedTaskbarResolver::resolveShortcutCached(const QString& shortcutPath) const
{
    const QString cacheKey = normalizePathKey(shortcutPath);
    const qint64 mtimeMs = QFileInfo(shortcutPath).lastModified().toMSecsSinceEpoch();

    const auto cached = m_shortcutCache.constFind(cacheKey);
    if (cached != m_shortcutCache.constEnd() && cached->mtimeMs == mtimeMs) {
        return cached->entry;
    }

    PinnedShortcutEntry entry = resolveShortcut(shortcutPath);
    m_shortcutCache.insert(cacheKey, ResolvedShortcutCacheEntry{ mtimeMs, entry });
    return entry;
}
...
QList<PinnedShortcutEntry> PinnedTaskbarResolver::collectFolderShortcuts() const
{
    ...
    auto appendShortcut = [&results, this](const QString& filePath) {
        ...
        results.push_back(resolveShortcutCached(filePath));
    };
...
{
    // collectFolderShortcuts() is the expensive step (filesystem walk of the pin
    // folders). Collect once and reuse it for both the favorite filtering and the
    // explicit-dock-pin merge instead of walking the folders twice.
    const QList<PinnedShortcutEntry> folderShortcuts = collectFolderShortcuts();
    const QList<PinnedShortcutEntry> windowsPins = filterPinnedShortcuts(folderShortcuts);
    if (explicitDockPinKeys.isEmpty()) {
        return windowsPins;
    }
...
{
    return filterPinnedShortcuts(collectFolderShortcuts());
}

QList<PinnedShortcutEntry> PinnedTaskbarResolver::filterPinnedShortcuts(const QList<PinnedShortcutEntry>& folderShortcuts) const
{
...
{
    const QList<PinnedShortcutEntry> pins = resolvePinnedShortcuts();
    emit logMessage(QStringLiteral("Taskbar sync scan: %1 pins from Windows taskbar registry.")
                        .arg(pins.size()));
    return pins;
}
